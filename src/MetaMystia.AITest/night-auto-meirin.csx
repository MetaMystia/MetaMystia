...
public static class AITestService
{
    ...
    public static string Start()
    {
        ...
        gm.StartCoroutine(Run(gm));
        return Status = "watching";
    }

    static IEnumerator Run(GuestsManager gm)
    {
        ...
        while (Time.realtimeSinceStartup < deadline)
        {
            for (int code = 0; code < gm.MaxDeskNum; code++)
            {
                ...
                if (so == null) continue;
                string foodTag = so.RequestFoodTag.GetFoodTag();
                string bevTag = so.RequestBeverageTag.GetBeverageTag();
