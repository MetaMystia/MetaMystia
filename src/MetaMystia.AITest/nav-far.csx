...
public static class AITestNav2
{
    public static string Status = "idle";
    public static bool Busy;

    public static string Go(float x, float y, float timeout = 16f)
    {
        ...
        Busy = true;
        p.StartCoroutine(Run(p, new Vector2(x, y), timeout));
        return "started";
    }

    static IEnumerator Run(DayScenePlayerInputGenerator p, Vector2 target, float timeout)
    {
        ...
        while (p != null && Time.realtimeSinceStartup < deadline)
        {
            ...
            if (delta.magnitude < 0.15f) { reason = "arrived"; break; }
        ...
        {
            p.ExternalStop();
            Status = reason + " | " + start + " -> " + p.transform.position;
        }
