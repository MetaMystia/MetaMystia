...
public static class AITestAutoServeV4
{
    ...
    static int FoodRecipe(int guestId, string tag)
    {
        if (guestId == 15)
        {
        ...
        if (guestId == 9000) return 11000;
        return 0;
    }

    static int BeverageId(int guestId, string tag)
    {
        if (guestId == 15)
        {
        ...
        if (guestId == 9000) return tag == "低酒精" ? 19 : 27;
        return 0;
    }
    ...
    static IEnumerator Run(GuestsManager gm, float deadline)
    {
        while (Time.realtimeSinceStartup < deadline)
        {
            ...
            for (int code = 0; code < gm.MaxDeskNum; code++)
            {
                ...
                if (guestId != 15 && guestId != 9000) continue;
                ...
                int foodRecipe = FoodRecipe(guestId, foodTag);
                int bevId = BeverageId(guestId, bevTag);
                if (foodRecipe == 0 || bevId == 0)
                {
                    Status = "unsupported guest=" + guestId + " foodTag=" + foodTag + " bevTag=" + bevTag;
                    continue;
                }
                Status = "serving guest=" + guestId + " desk=" + code + " foodTag=" + foodTag + " bevTag=" + bevTag;
