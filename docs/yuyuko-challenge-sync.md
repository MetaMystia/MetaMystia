...
| 阶段结束 | `Timing\|2`、主循环恢复点 | 客机等待主机结束消息，带入主机营业额、符卡数和生命值后运行原版判定与收尾 |
...
`YuyukoTimingPatch` 保留客机本地计时显示：本地先结束则等待主机消息，主机消息先到则结束本地计时。`YuyukoMainLoopPatch` 随后在对应恢复位置回填主机数据，再放行原版判定。主循环补丁返回的 `SkipOriginal` 只暂停本次执行，配合 `__result = true` 保持协程存活；计时补丁的 `__result = false` 则表示计时协程结束。
...
- 尚未双端游戏实测。需要验证本体双方上菜、分身差评吞厨具、厨具正在烹饪时被吞、阶段结束与吞食重叠、剧情播放速度不同的完整通关和失败流程。
