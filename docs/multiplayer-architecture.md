...
| `RoomClock` | 本地计时和房间延迟估算；与网络存活检测分开 |
...
- 世界资料包含名字、场景、玩法阶段和皮肤信息；资源表只在同房间共享，连接期间固定。新成员接收完整资源，已有成员只接收新入房者的资源；后续快照省略已发送的资源，由客户端补齐。退房后清理缓存，再次入房重新初始化。缺失自定义皮肤时沿用本地回退显示。
...
| 选店、准备 | 菜谱、酒水和厨具在注册或注销时逐项提交，预设共用这些入口；房主按接收顺序应用并广播完整结果，转场期间缓存准备数据 |
...
游戏、模组和协议版本统一配置在根目录 `Versions.props`。协议调整后，参与者需一起更新。
