...
[场景流转图](scene-flow-audit.md) 中蓝色标记表示同步点，红色路径表示先退房再执行原操作。独立服务器保留世界连接；局域网退房即断线，房主退房会关闭服务器。正常结算或试炼正常返回保留房间。
...
| 共同流程读档、回菜单、回退 | 在 `SaveManagement` 重置游戏状态前主动退房，取消等待中的续接 |
| 未接入同步的夜间入口、共同流程结局改道 | 在 `UniversalGameManager.LoadScene` 执行前退房；最终试炼的 `TryLeaveSession` 正常返回单独放行 |
