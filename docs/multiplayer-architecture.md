...
| 确认前读档或返回菜单 | 撤回本人意向；若与房主提议交错，拒绝并取消该次提议 |
| 已接受共同入口 | 结束白天事件、选店均属于共同流程，读档按中断处理 |
...
玩家资料发布玩法阶段，不比较日期。TCP 保证同一连接的消息顺序，入口确认处理撤回与提议的交错；入口协调使用 `DayDestinationManager.Round`，试炼备菜使用准备编号，没有统一的玩法轮次。
...
游戏、模组和协议版本统一配置在根目录 `Versions.props`。当前协议为 `3`，与旧协议不兼容，参与者需一起更新。
